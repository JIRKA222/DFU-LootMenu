    ...
    namespace LootMenuMod
    {
        public class LootMenu : MonoBehaviour
        {
                ...
                if (enableLootMenu && GameManager.Instance.IsPlayerOnHUD)
                {
                    ...
                    for (int i = 0; i < itemNameList.Count; i++)
                    {
                        if (i < numberOfItemsShown && currentItem < itemNameList.Count)
                        {
                            ...
                            {
                                newItemFontSize = GetItemMaxFontSize((int)(itemWidth * 1.1f), true);
                                itemPos = GetItemTextPoS(itemWidth, i, newItemFontSize);
            ...
            {   
                return new Vector2(backgroundUseablePos[0] + (titleSize[0] - titleWidth * fontSize[0]) / 2, backgroundUseablePos[1] + (titleSize[1] - (daggerfallFont0001.GlyphHeight * fontSize[1]) * 1.5f) / 2 + titlePosYOffset);
            }
            ...
            {
                return new Vector2(backgroundUseablePos[0] + (itemSize[0] - itemWidth * fontSize[0]) / 2, backgroundUseablePos[1] + titleSize[1] + itemSize[1] * index + (itemSize[1] - daggerfallFont0003.GlyphHeight * fontSize[1]) / 2 + itemPosYOffset);
            }
